...
<body>
	<%
		...
		String v_Model=request.getParameter("vModel");
		String numberPlate=request.getParameter("chassis");
		String sType=request.getParameter("sType");
		String lastService=request.getParameter("lastService");
		String sDate=request.getParameter("sDate");
		String sTime=request.getParameter("sTime");
		String answer=request.getParameter("pickupDrop");
		String issues=request.getParameter("issue");
  ...
  <div class="form-container">
	...
	<form action="updateBookingServiceServlet" method="POST">
	
	  <label>Booking ID:</label>
	  <input type="text" id="id" name="bookingID" value="<%= id %>" readonly>
	  
	  <label>Customer ID:</label>
	  <input type="text" id="id" name="customerID" value="<%= cusid %>" readonly>
	  
	  <h2>📋 Customer Information</h2>
	  <div class="form-row">
	    <div class="form-group">
	      <label>👤 Full Name</label>
	      <input type="text" id="fullName" name="full_name" value="<%= fname %>" >
	    </div>
	    <div class="form-group">
	      <label>📞 Phone Number</label>
	      <input type="tel" id="phone" name="phone_number" value="<%= phone %>" >
	    </div>
	  </div>
	  <div class="form-group">
	    <label>📧 Email Address</label>
	    <input type="email" id="email" name="email_address" value="<%= email %>" >
	  </div>
	    ...
	    <div class="form-group">
	      <label>🚙 Vehicle Type</label>
			<select id="vehicleType" name="vehicle_type">
			  <option value="" <%= (vType == null || vType.isEmpty()) ? "selected" : "" %>>-- Select Vehicle Type --</option>
			  ...
			  <option value="van" <%= "van".equals(vType) ? "selected" : "" %>>Van</option>
			  <option value="other" <%= "other".equals(vType) ? "selected" : "" %>>Other</option>
			</select>
		</div>
	    <div class="form-group">
	      <label>🚘 Vehicle Model</label>
	      <input type="text" id="vehicleModel" name="vehicle_model" value="<%= v_Model %>" >
	    </div>
	  </div>
	  <div class="form-group">
	    <label>🔢 Number Plate</label>
	    <input type="text" id="plate" name="number_plate" value="<%= numberPlate %>"  >
	  ...
	  <div class="form-group">
		...
		<select id="serviceType" name="service_type">
		  <option value="" <%= (sType == null || sType.isEmpty()) ? "selected" : "" %>>-- Select Service Type --</option>
		  <option value="maintenance" <%= "maintenance".equals(sType) ? "selected" : "" %>>Maintenance</option>
		  <option value="oil_change" <%= "oil_change".equals(sType) ? "selected" : "" %>>Oil Change</option>
		  <option value="repair" <%= "repair".equals(sType) ? "selected" : "" %>>Repair</option>
		  <option value="inspection" <%= "inspection".equals(sType) ? "selected" : "" %>>Inspection</option>
		</select>
	  ...
	  <div class="form-row">
	    <div class="form-group">
	      ...
	      <input type="date" id="lastService" name="last_service_date" value="<%= lastService %>">
	    </div>
	    <div class="form-group">
	      <label>📅 Service Date</label>
	      <input type="date" id="serviceDate" name="service_date" value="<%= sDate %>">
	    </div>
	    <div class="form-group">
	      <label>⏰ Service Time</label>
	      <input type="time" id="serviceTime" name="service_time" value="<%= sTime %>">
	    </div>
	  </div>
	  <div class="form-group">
	    <label>🚗 Pickup & Drop Required?</label>
		<select id="pickupDrop" name="pickup_drop">
		  <option value="" <%= (answer == null || answer.isEmpty()) ? "selected" : "" %>>-- Select Option --</option>
		  <option value="yes" <%= "yes".equals(answer) ? "selected" : "" %>>Yes</option>
		  <option value="no" <%= "no".equals(answer) ? "selected" : "" %>>No</option>
		</select>

	  </div>
	  <div class="form-group">
	    <label>📝 Describe Issues (if any)</label>
	    <textarea id="issues" name="vehicle_issues"><%= issues %></textarea>
	  </div>
